...
class BertEncoder(nn.Module):
    def __init__(self,
        ...
        self.bert_type = "/XXX/XXX/XXX/XXX/pretrained_models/PathologyBERT"
        self.last_n_layers = 1
        self.aggregate_method = "sum"
        self.embedding_dim = emb_dim
        self.output_dim = output_dim
        self.freeze_bert = freeze_bert
        self.agg_tokens = True # 合并子词

        self.config = BertConfig.from_json_file(
            os.path.join(BASE_DIR, "../../configs/config.json"))
    ...
    def forward(self, ids, attn_mask, token_type, get_local=False):
        outputs = self.model(ids, attn_mask, token_type, output_attentions=True,
