...
from mgca.datasets.pretrain_dataset_textbook import (MultimodalPretrainingDataset,
...
def cli_main():
    ...
    experiment_name = (args.experiment_name + "&带缓冲层&VF对齐&语义分类&60epoch&textbook描述")

    # get current time
    now = datetime.datetime.now(tz.tzlocal())
    extension = now.strftime("%Y_%m_%d_%H_%M_%S")
    ckpt_dir = os.path.join(
        BASE_DIR, f"../../../data/ckpts/semantic_vf_textbook/{experiment_name}_{extension}")
