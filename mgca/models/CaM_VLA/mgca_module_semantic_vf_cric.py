...
from mgca.datasets.pretrain_dataset_CRIC import (MultimodalPretrainingDataset,
...
class MGCA(LightningModule):
    ...
    def __init__(self,
        ...
        self.class_names = ["ASC-H", "ASC-US", "HSIL", "LSIL", "NILM", "SCC"]
...
def cli_main():
    ...
    experiment_name = (args.experiment_name + "&带缓冲层&VF对齐&语义分类&50epoch&CRIC欠采样2000数据集")

    # get current time
    now = datetime.datetime.now(tz.tzlocal())
    extension = now.strftime("%Y_%m_%d_%H_%M_%S")
    ckpt_dir = os.path.join(
        BASE_DIR, f"../../../data/ckpts/semantic_vf_CRIC/{experiment_name}_{extension}")
